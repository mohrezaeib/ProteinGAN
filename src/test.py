...
def fasta_to_pandas(path, separator=";"):
    ...
    with open(path) as fasta_file:
        ...
        for title, sequence in SimpleFastaParser(fasta_file):
            ...
            titles.append("|".join(title_parts[0:]))
            sequences.append(sequence)
            labels.append("labelx")
            id +=1
        return pd.DataFrame({"id": identifiers, "title": titles,"Label":labels, "sequence": sequences})
df = fasta_to_pandas("/home/mohre/B/d/Research/templates/ProteinGAN/src/a/1.fasta")
save_as_tfrecords_multithreaded("/home/mohre/B/d/Research/templates/ProteinGAN",df)